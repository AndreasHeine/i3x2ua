...
from i3x_server.application.services.subscription_mapper import (
    ...
    map_public_subscription_updates,
...
def _resolve_model_nodes(model: BuildResult, element_ids: list[str]) -> list[tuple[str, ModelNode | None]]:
...
def _raise_not_found(entity_type: str, entity_id: str) -> Never:
    """Raise a 404 error for a missing entity."""
    raise i3x_http_error(
        404,
        "NotFound",
...
def _raise_invalid_argument(
    ...
    """Raise a 400 error for invalid input."""
    if message is None:
...
def _raise_opcua_error(operation: str, cause: Exception | str) -> Never:
    """Raise a 502 error for OPC UA operation failure."""
...
async def _fetch_namespace_infos(
    opcua_client: OpcUaClientProtocol,
) -> list[OpcUaNamespaceInfo]:
    try:
        return await opcua_client.get_namespace_infos()
    except Exception:
        # Metadata fetching should not fail operations; return empty list and continue gracefully.
        return []


def _good_no_data_vqt() -> VQT:
    """Construct a VQT with null value and GoodNoData quality (normalized timestamp)."""
...
def _map_subscription_bulk_items_to_result_items(
    ...
    """Convert subscription mapper DTOs to API bulk response items."""
    return [
        BulkResultItem[T](
...
def _map_subscription_result(
    ...
    """Map a subscription result using optional mapper function."""
    if result is None:
        return None
...
async def get_namespaces_v1(
    ...
    try:
        ...
        _raise_opcua_error("read namespaces", exc)
...
async def sync_subscription_v1(
    ...
    batch_payload = map_public_sync_batches(
        synced,
        element_id_mapper=lambda element_id: _expanded_node_id(element_id, namespace_infos),
        value_mapper=_to_json_safe_value,
    )
    result_payload = [
        SyncBatch(
            sequenceNumber=item["sequenceNumber"],
            updates=[SyncUpdate(**update) for update in item["updates"]],
...
async def delete_subscriptions_v1(
    ...
    return _bulk_response(_map_delete_subscription_bulk_result_items(map_delete_subscription_items(deleted)))
...
async def list_subscriptions_v1(
    ...
    subscriptions = await subscription_service.list_subscriptions(client_id, body.subscriptionIds or None)
    items = map_subscription_detail_bulk_items(
        subscriptions,
        requested_ids=body.subscriptionIds,
        element_id_mapper=lambda element_id: _expanded_node_id(element_id, namespace_infos),
    )
    return _bulk_response(_map_subscription_detail_bulk_result_items(items))
