...
class ConformanceFixtureServer:
	...
	async def _set_historizing_flags(self, node: Any) -> None:
		...
		await node.write_attribute(ua.AttributeIds.AccessLevel, data_value)
		await node.write_attribute(ua.AttributeIds.UserAccessLevel, data_value)
		historizing = ua.DataValue(ua.Variant(True, ua.VariantType.Boolean))
		await node.write_attribute(ua.AttributeIds.Historizing, historizing)
